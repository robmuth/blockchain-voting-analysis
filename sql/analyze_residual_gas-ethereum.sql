...
CREATE TEMP FUNCTION measureNeededBlocks (gas INT64, startDate DATE)
  RETURNS INT64 AS ( (
    SELECT
      number
    FROM (
      SELECT number, SUM(LEAST(gas_limit, 15 * POW(10, 6)) - gas_used) OVER (ORDER BY number DESC) AS gas_free_sum,
...
LOOP
  ...
  VALUES
    ...
    foundBlock,
    (SELECT CAST(AVG(LEAST(gas_limit, 15 * POW(10, 6))) AS INT64) FROM `bigquery-public-data`.crypto_ethereum.blocks b WHERE b.number >= foundBlock AND b.number <= (SELECT MAX(bc.number) FROM `bigquery-public-data`.crypto_ethereum.blocks bc WHERE date(bc.timestamp) = day))
  );
